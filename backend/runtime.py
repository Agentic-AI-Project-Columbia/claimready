...
def build_extractor() -> Agent:
    return Agent(
        ...
        instructions=(
            ...
            "CRITICAL: You MUST leave the following defendant fields EMPTY — they are "
            "populated later by a separate agent that calls the official NY DOS registry: "
            "defendant.dos_entity_name, defendant.dos_id, defendant.service_address, "
            "defendant.registered_agent, defendant.entity_status. Even if the evidence "
            "mentions an address or agent name, do NOT put it in any of those fields. "
            "Only set defendant.name (and any plain Party fields like phone/email if present)."
        ),
        model=_model(),
...
def build_planner() -> Agent:
    ...
    planner = Agent(
        ...
        instructions=(
            ...
            "Your job is to coordinate handoffs in EXACTLY this order — every step is mandatory, "
            "even if you think the data already looks complete:\n"
            ...
            "  2. ALWAYS hand off to DefendantResolver next, as long as defendant.name is set. "
            "     Do this regardless of whether dos_id appears populated — the Extractor is not "
            "     allowed to set DOS fields, so any DOS data in the running CaseFacts at this "
            "     point is unreliable and must be replaced by DefendantResolver's verified output.\n"
